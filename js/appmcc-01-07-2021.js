...
*/
    ...
    function decodeHash() {
        ...
        if (hash) {
            let data = atob(hash).split('&');
            let id_barrio = data[1];

            localStorage.setItem('barrio', id_barrio);
            document.getElementById('quit_filtro_barrio').classList.remove('d-none');

            saveVisit(data[1], data[0]); //En la posicion 0 esta el nombre de barrio, en la uno su ID
            
            //Consulta y pinta el poligono del barrio en el mapa ----------------------------------------------------------------
            fetch('rec_elem.php?queBusca=Barrio&nombre_calle=' + id_barrio)
                ...
                .then(response => {
                    ...
                    map.setView(geojson.getBounds().getCenter(), 16);
                })
                .catch(err => console.log(err));
            // -----------------------------------------------------------------------------------------------------------

            //Consulta obras correspondientes al barrio ------------------------------------------------------------------
            //localhost/idemcc/?h=Q0VOVFJPJjE3&menu=false
            //getObras(null, id_barrio)
    ...
    function getObras(idobra, idbarrio) {

        if (!idobra) idobra = '';
        if (!idbarrio) idbarrio = '';

        fetch('obras.php?b=' + idbarrio + '&obra=' + idobra)
        .then(res => res.json())
        .then(res => {
    ...
    function prenderObra(idobra) {
        let idbarrio = localStorage.getItem('barrio')
        if (idbarrio) getObras(idobra, idbarrio)
    ...
    document.getElementById('quit_filtro_barrio').addEventListener('click', function () {
        this.style.display = 'none';
        localStorage.removeItem('barrio');
    ...
    function saveVisit(id_barrio, nombre_barrio){
        /* 
        let position = await getCurrentPosition(); */
        let dispositivo = window.mobileCheck() ? 'mobile' : 'desktop';
        let fm = new FormData()
        fm.append('url', location.href)
        fm.append('id_barrio', id_barrio)
        fm.append('nombre_barrio', nombre_barrio)
        fm.append('dispositivo', dispositivo);

        /* if(position){
            fm.append('lat', position.coords.latitude);
            fm.append('lng', position.coords.longitude); 
        } */

        fetch(location.origin+'/app-qr-counter/api.php?query=create', {
            method: 'POST',
            body: fm
        })
        .then(res=>res.json())
