...
*/
    ...
    function decodeHash() {
        ...
        if (hash && hash != '') {
            ...
            localStorage.setItem('zona', n_zona);
            document.getElementById('quit_filtro_zona').classList.remove('d-none');

            saveVisit(n_zona, data[0]); //En la posicion 0 esta el nombre de zona, en la uno n_zona
            
            //Consulta y pinta el poligono dela zona en el mapa ----------------------------------------------------------------
            fetch('zona.php?query=zona&n_zona=' + n_zona)
            ...
            //Consulta obras correspondientes a la zona ------------------------------------------------------------------
    ...
    function getObras(idobra, n_zona) {

        if (!idobra) idobra = '';
        if (!n_zona) n_zona = '';

        fetch('obras.php?n_zona=' + n_zona + '&obra=' + idobra)
        .then(res => res.json())
        .then(res => {
    ...
    function prenderObra(idobra) {
        let n_zona = localStorage.getItem('zona')
        if (n_zona) getObras(idobra, n_zona)
    ...
    document.getElementById('quit_filtro_zona').addEventListener('click', function () {
    ...
    function saveVisit(n_zona, nombre_zona){
        /* 
        let position = await getCurrentPosition(); */
        let dispositivo = window.mobileCheck() ? 'mobile' : 'desktop';
        let fm = new FormData()
        fm.append('url', location.href)
        fm.append('n_zona', n_zona)
        fm.append('nombre_zona', nombre_zona)
        fm.append('dispositivo', dispositivo)

        fetch(location.origin+'/app-qr-counter/api_zona_qr.php?query=create', {
            method: 'POST',
            body: fm
        })
        .then(res=>res.json())
